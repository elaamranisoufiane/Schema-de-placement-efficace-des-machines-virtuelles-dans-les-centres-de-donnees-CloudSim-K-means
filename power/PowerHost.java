...
public class PowerHost extends HostDynamicWorkload {
	...
	///////////////////////////
		...
		Comparator T= new Comparator<PowerHost>() {

			public int compare(PowerHost a, PowerHost b) throws ClassCastException{
				 Double CurrentPower = getAvailablePower(a);
				 Double  MaxPower = getAvailablePower(b);
				return MaxPower.compareTo(CurrentPower);
			}};
	public static  double getAvailablePower(PowerHost host) {
		//Collections.sort(vmList, new Comparator<T>()	
		 return (Double)host.getMaxPower()-host.getPower();
	}
